<script lang="ts">
	...
	function getRandomProduct() {
		product = $productsStore[getRndInt(0, $productsStore.length)];
	}
</script>

<div id="suprise">
	{#if product}
		<a in:fly={{ y: -20, duration: 600 }} href="/products/{product.id}">
			<div class="card">
	...
	<div id="suprise-header">
		{#key product}
			<span
				in:fly={{ y: -20, delay: 300, duration: 600 }}
				class="headliner"
				>{product
					? product.title
		...
		{/key}
		<button
			style="display: {product ? 'none' : 'block'};"
			in:fly={{ y: 20, delay: 200, duration: 600 }}
			on:click={() => getRandomProduct()}>узнать</button
		>
	</div>
</div>
